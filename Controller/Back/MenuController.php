...
class MenuController extends AbstractController
{
    ...
    public function index(PaginatorInterface $paginator, Request $request, MenuRepository $menuRepository): Response
    {
        ...
        if ($request->query->get('search')) {
            $query->leftJoin('a.menuArea', 'menuArea')->andWhere('a.title LIKE :keyword OR a.slug LIKE :keyword OR menuArea.name LIKE :keyword')->setParameter('keyword', '%' . $request->query->get('search') . '%');
        }
        $els = $paginator->paginate($query->getQuery(), $request->query->getInt('page', 1), 12);
        return $this->render('@AkyosCms/crud/index.html.twig', ['els' => $els, 'title' => 'Menu', 'route' => 'menu', 'fields' => ['ID' => 'Id', 'Titre' => 'Title', 'Slug' => 'Slug', 'Zone de menu' => 'MenuArea'],]);
    }
    ...
    public function new(Request $request, EntityManagerInterface $entityManager): Response
    {
        ...
        return $this->render('@AkyosCms/menu/new.html.twig', ['el' => $menu, 'title' => 'Menu', 'route' => 'menu', 'form' => $form->createView(),]);
    }
    ...
    public function edit(Request $request, Menu $menu, MenuItemRepository $menuItemRepository, EntityManagerInterface $entityManager): Response
    {
        ...
        return $this->render('@AkyosCms/menu/edit.html.twig', ['el' => $menu, 'title' => 'Menu', 'route' => 'menu', 'menuItems' => $menuItemRepository->findBy(['menu' => $menu->getId()], ['position' => 'ASC']), 'formItem' => $formItem->createView(), 'form' => $form->createView(),]);
    }
     ...
     * @return Response
     */
    #[Route(path: '/{id}', name: 'menu_delete', methods: ['DELETE'])]
     ...
     */
    #[Route(path: '/{id}/item/change-position', name: 'menu_change_position_menu_item', methods: ['POST'], options: ['expose' => true])]
